...
export function siteNav(active: 'place' | 'volume' | 'ads' | 'blog', dark = false): string {
  ...
  return `<div class="site-head ${dark ? 'on-dark' : ''}"><div class="site-head-in">
    ...
    <nav class="site-nav">
      ${item('place', '/', '플레이스분석')}
      ${item('volume', '/keyword-volume', '키워드 검색량')}
      ${item('ads', '/ads', '광고컨설팅')}
  ...
  return `<footer class="site-foot"><div class="wrap">
    <div class="cols">
      ...
      <div>
        <h4>무료 도구</h4>
        <a href="/">내 매장 순위 진단</a>
        <a href="/keyword-volume">키워드 검색량</a>
...
export function renderKeywordVolumePage(): string {
  ...
  return shell({ title: '네이버 키워드 검색량 조회 | 동네장사', description: '네이버 키워드 검색량 조회 도구입니다. 검색어별 월간 PC·모바일 검색량, 합계와 관련 키워드를 네이버 검색광고 API 기준으로 확인하세요.', canonical: 'https://dongbiz.com/keyword-volume', active: 'volume', style: KEYWORD_VOLUME_STYLE, body });
}
